		...
		if (count($cmr_array) >= 2) {
			...
			if ($cmr_mean != 0.0) {
				...
				if( count($x_coordinates) == count($y_coordinates) ){
					array_push($lj_components, $cmr_array, $limits, $x_coordinates, $response);
					echo json_encode (array("lj_components" => $lj_components));
		    	} else if (count($x_coordinates) != count($y_coordinates)) {
		    		$response["error"] = TRUE;
		    		$response["error_message"] = "Oops, something is not right. Please try again.";
		    		array_push($lj_components, $cmr_array, $limits, $x_coordinates, $response);
		    		echo json_encode(array("lj_components" => $lj_components));
		    	}
			}
		} else if (count($cmr_array) < 2) {
			$response["error"] = TRUE;
			$response["error_message"] = "Failed. Not enough control material results for $cm_name";
			array_push($lj_components, $cmr_array, $limits, $x_coordinates, $response);
			echo json_encode(array("lj_components" => $lj_components));
		}
	} else {
